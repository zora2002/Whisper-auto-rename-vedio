...
3. 移除中文標點（`。！？；：、`）及英文標點（`, . ! ? ; :`）
4. 組合新檔名：`<safeName>_<原始檔名（不含副檔名）><副檔名>`
5. 若目標檔名已存在，加上數字後綴（`_1`、`_2`…）避免衝突
6. 執行 `fs.renameSync` 完成重命名
7. 回傳重命名後的完整路徑（供後續移動至結果資料夾使用）

> **重要**：renamer 收到的 `inputFile` 一定已是標準格式 `HHMMSS_YYYYMMDD.mp4`。
> Screen Recording 格式由 file-preprocessor 預先轉換，renamer 不需處理原始 Screen Recording 檔名。
